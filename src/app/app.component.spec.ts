...
describe('BookDetailsComponent', () => {
  ...
  it('validation of books between 600-3000', async() => {
    await pushInputValue(min_price, 600);
    await pushInputValue(max_price, 3000);
    filter_button = getByTestId('filter_button');
    filter_button.click()
    await fixture.detectChanges();
    await fixture.whenStable();
    expect(msg).toBeNull();
    let buttonLabels = fixture.debugElement.queryAll(By.css('.custom-margin'));
    expect( buttonLabels.length).toBe(1);
  });
  ...
  it('validation for no book in price range', async() => {
    await pushInputValue(min_price, 10);
    await pushInputValue(max_price, 20);
    filter_button = getByTestId('filter_button');
    filter_button.click()
    await fixture.detectChanges();
    await fixture.whenStable();
    ...
    let buttonLabels = fixture.debugElement.queryAll(By.css('.custom-margin'));
    expect( buttonLabels.length).toBe(0);
  });

  it('validation for price range', async() => {
    await pushInputValue(min_price, 21);
    await pushInputValue(max_price, 20);
    filter_button = getByTestId('filter_button');
    filter_button.click()
    await fixture.detectChanges();
    await fixture.whenStable();
    let error_msg = fixture.debugElement.queryAll(By.css('.text-danger'));
    expect( error_msg.length).toBe(1);

    await pushInputValue(min_price, 21);
    await pushInputValue(max_price, 22);
    filter_button = getByTestId('filter_button');
    filter_button.click()
    await fixture.detectChanges();
    await fixture.whenStable();
    error_msg = fixture.debugElement.queryAll(By.css('.text-danger'));
    expect( error_msg.length).toBe(0);
  });
});